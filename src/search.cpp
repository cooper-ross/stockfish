...
moves_loop:  // When in check, search starts here
    ...
    // A fail low in a singular exclusion search only bounds the position
    // without its best move, so it says nothing about the static evaluation.
    if (!ss->inCheck && !(bestMove && pos.capture(bestMove)) && (bestMove || !excludedMove)
